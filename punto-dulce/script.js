...
/* ---------------- SESIÓN DE EMPLEADO (API + MySQL) ----------------
   El registro, login y roles del personal ya NO se guardan en este
   navegador: viven en la base de datos MySQL, detrás de la API en
...
function renderEmployeeOrders(){
  ...
  const orders = getOrders();
  if(orders.length===0){
    tbody.innerHTML = '<tr><td colspan="6" class="hint" style="padding:16px 0;">No hay pedidos registrados todavía.</td></tr>';
    return;
  }
