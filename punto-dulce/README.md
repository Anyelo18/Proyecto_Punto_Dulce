...
├── empleado-login.html   → Registro / inicio de sesión del personal (contra la API)
...
├── backend/                 → API Node.js + Express + MySQL (registro, login y roles del personal)
│   ├── server.js
│   ├── db.js
│   ├── schema.sql
│   ├── package.json
...
Necesitas **dos cosas corriendo a la vez**: el backend (API + MySQL) y el sitio (Live Server). Usa dos terminales.
...
Deberías ver: `Punto Dulce API escuchando en http://localhost:4000`.
...
- El carrito, los pedidos, el catálogo editable y la materia prima se guardan en `localStorage` del navegador. **Solo** el registro, login y roles del personal usan MySQL a través de la API en `backend/`.
